		...
		public void ClientRender(EntityArray entityArray, Entity commandingEntity)
		{
			if (commandingEntity.IsValid && commandingEntity.HasComponent<SpatialComponent>())
			{
				...
				foreach (Entity entity in entityArray)
				{
					...
					if (modelComponent.ModelName == "" || !this.Models.TryGetValue(modelComponent.ModelName, out Model model)) { continue; }

					this.BasicEffect.Texture = model.Texture;
					this.BasicEffect.TextureEnabled = (model.Texture != null);
					...
					this.GraphicsDeviceManager.GraphicsDevice.DrawUserPrimitives<VertexPositionNormalTexture>(PrimitiveType.TriangleList, model.Vertices, 0, model.Vertices.Length / 3);
				}
			}
		}
