...
{
	public class FpsMonoGame : Microsoft.Xna.Framework.Game
	{
		#region Fields
		...
		public FpsMonoGame(IPAddress ipAddressToConnectTo)
		{
		...
		protected override void Initialize()
		{
			...
			IServerSystem[] serverSystems = new IServerSystem[] { new PlayerSystem(), };
			IClientSystem[] clientSystems = new IClientSystem[] { new PlayerSystem(), this.renderSystem = new RenderSystem(this.graphicsDeviceManager) };
		...
		private int updateCommandingEntityID(bool isClientConnected, int currentCommandingEntityID)
		{
			if (isClientConnected && currentCommandingEntityID == -1)
			{
				if (this.gameServer.EntityArray.TryCreateEntity(out Entity clientEntity))
				{
					...
					spatialComponent.Extents = new Vector3(16, 70, 16);
